                        ...
                        loadingList.map((product, index) => {
                            return (
                                <div className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
                                    <div className='bg-slate-200 h-full p-4 min-w-[120px] md:min-w-[140px] animate-pulse'>
                                       
                                    </div>
                                    <div className='p-4 grid gap-2 w-full'>
                                        <h2 className='font-medium text--base md:text-lg text-ellipsis line-clamp-2 text-black bg-slate-200 animate-pulse p-1 rounded-full'> </h2>
                                    ...
                                    <Link to={"product/"+product?._id} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
                                        ...
                                        <div className='p-4 grid gap-1'>
                                            ...
                                            <div className='flex gap-3 '>
                                                <p className='text-red-600 font-medium text-sm'>{displayBDCurrency(product?.sellingPrice)}</p>
                                                <p className='text-slate-400 line-through text-sm'>{displayBDCurrency(product?.price)}</p>
                                            </div>
                                            <div className='flex justify-center items-center'>
                                                <button className='w-24 items-center justify-center text-sm bg-red-500 hover:bg-red-700 text-white px-3 py-0.5 rounded-full' onClick={(e)=>handleAddToCart(e,product?._id)}>Add to Cart</button>
                                            </div>
                                        </div>
